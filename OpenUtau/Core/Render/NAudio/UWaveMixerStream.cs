...
{
    class UWaveMixerStream : WaveStream
    {
        ...
        /// </summary>
        public UWaveMixerStream() : this(WaveFormat.CreateIeeeFloatWaveFormat(44100, 2)) { }

        /// <summary>
        /// Creates a new WaveMixerStream
        /// </summary>
        public UWaveMixerStream(WaveFormat format)
        {
            AutoStop = true;
            waveFormat = format;
        ...
        /// <exception cref="ArgumentException">Thrown if the input streams have different formats and cannot be convert to the same format</exception>
        public UWaveMixerStream(IEnumerable<WaveStream> inputStreams, bool autoStop)
            : this(inputStreams, WaveFormat.CreateIeeeFloatWaveFormat(44100, 2), autoStop)
        {
            ...
            /// <param name="autoStop">Automatically stop when all inputs have been read</param>
            /// <exception cref="ArgumentException">Thrown if the input streams have different formats and cannot be convert to the same format</exception>
            public UWaveMixerStream(IEnumerable<WaveStream> inputStreams, WaveFormat format, bool autoStop)
            : this(format)
        {
            AutoStop = autoStop;
        ...
        public void AddInputStream(WaveStream waveStream)
        {
            ...
            //    throw new ArgumentException("Only 32 bit audio currently supported", "waveStream");
                ...
                catch (Exception e)
                {
                    ...
                    throw new ArgumentException($"cannot convert waveStream into {waveFormat.ToString()}",e);
                }
                }
        ...
        /// Actually performs the mixing
        /// </summary>
        static unsafe void SumAudio(byte[] destBuffer, int offset, byte[] sourceBuffer, WaveFormat format, int bytesRead)
        {
            fixed (byte* pDestBuffer = &destBuffer[offset],
                      pSourceBuffer = &sourceBuffer[0])
            {
                if (format.Encoding == WaveFormatEncoding.IeeeFloat)
                {
                    float* pfDestBuffer = (float*)pDestBuffer;
                    float* pfReadBuffer = (float*)pSourceBuffer;
                    int samplesRead = bytesRead / (format.BitsPerSample / 8);
                    for (int n = 0; n < samplesRead; n++)
                    {
                        pfDestBuffer[n] += pfReadBuffer[n];
                    }
